...
The "Online (Natural)" voices (such as Microsoft Ava, Andrew, and Jenny) provide significantly higher quality than the legacy voices, and are auto-picked when available.
...
**Linux users:** Chromium on Linux often only ships Google voices. They work for full-page reading (sentence focus, colors, dim modes are unaffected) but do not expose word boundaries, so the per-word marker is automatically suppressed when one is active. For per-word highlighting, install `espeak` or `speech-dispatcher` via your package manager, or use the Kokoro backend below.
