...
**Linux users:** Chromium on Linux often only ships Google voices. They work for full-page reading (active text/sentence focus, colors, dim modes are unaffected) but do not expose word boundaries, so the per-word marker is suppressed and active-line focus falls back to active-text when one is active (their rate is also capped at 2x). For per-word highlighting and faster playback, install `espeak` or `speech-dispatcher` via your package manager, or use the Kokoro backend below.
